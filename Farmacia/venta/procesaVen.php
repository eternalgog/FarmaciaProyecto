...
<body>
  ...
  <nav class="navbar navbar-expand-lg navbar-dark fixed-top">
    <div class="logo">
      ...
      <a href=""><img width="50px" height="auto" src="capsulas.png"></a>
    </div class="nameL">
    ...
    <?php 
        ...
        $sql = "SELECT * FROM medicamentos WHERE nombre='$med'";
        $result = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_assoc($result)){
         $precio = $row['precio'];
         $cantidadOG = $row['cantidad'];
        }
        $total = (float)$precio * (int)$cantidad;

            // INSERTAR DATO
            $sql = "INSERT INTO venta (medicamento,cliente,cantidad,total)

            VALUES
            (
            '$med','$cli','$cantidad','$total'
            )";
            if ($conn->query($sql) == TRUE )
            {
                echo "<br> Total de venta = $$total <br>
                <p>Registro exitoso</p>";
            }
            $cantidadN = (int)$cantidadOG - (int)$cantidad;
            $sql = "UPDATE `medicamentos` SET `cantidad` = '$cantidadN' WHERE `medicamentos`.`nombre` = '$med';";
            $result = mysqli_query($conn,$sql);
            mysqli_close($conn);
