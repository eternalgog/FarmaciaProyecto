...
<body>
  ...
  <nav class="navbar navbar-expand-lg navbar-dark fixed-top">
    ...
    <div class="container text-light bg-dark text-center">
        <div>
            <h1>
              <br> Eliminar proveedor</h1>
        </div>

        <!-- Inicio del formulario-->
        <form class="bg-dark" action="procesaEliminar.php" method="POST" enctype="multipart/form-data">
        <div class="form-group mb-3">
              <label for="nombre" >ID del proveedor</label>
              <input class="form-control" type="text" class="form-control" name="id" id="id" readonly>
            </div> 
            <div class="form-group mb-3">
              <label for="nombre" >Nombre del proveedor</label>
              <input class="form-control" type="text" class="form-control" name="nombre" id="nombre" readonly>
            </div>
            <div class="form-group mb-3">
              <label for="uso" >Telefono</label>
              <input class="form-control" type="text" class="form-control" name="uso" id="uso" readonly>
            </div>             
        ...
        $id = $_GET['id'];
        $sql2 = "SELECT * FROM proveedores WHERE idProveedor=$id";
        ...
        while($row = mysqli_fetch_assoc($result2)){
          ...
          'document.getElementById("uso").value = "',$tlf,'";',
          '</script>';
